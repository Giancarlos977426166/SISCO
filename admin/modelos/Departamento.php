...
public function insertar($nombre,$descripcion,$idusuario){
	...
	$sql="INSERT INTO departamento (nombre,descripcion,user_create) VALUES ('$nombre','$descripcion','$idusuario')";
	return ejecutarConsulta($sql);
}

public function editar($iddepartamento,$nombre,$descripcion,$idusuario){
	$sql="UPDATE departamento SET nombre='$nombre',descripcion='$descripcion',user_create='$idusuario' 
	WHERE iddepartamento='$iddepartamento'";
	return ejecutarConsulta($sql);
}

public function borrar($iddepartamento){
	$sql="DELETE FROM departamento WHERE iddepartamento='$iddepartamento'";
	return ejecutarConsulta($sql);
}

public function desactivar($iddepartamento){
	$sql="UPDATE departamento SET fecha_create='0' WHERE iddepartamento='$iddepartamento'";
	return ejecutarConsulta($sql);
}

public function activar($iddepartamento){
	$sql="UPDATE departamento SET fecha_create='1' WHERE iddepartamento='$iddepartamento'";
	return ejecutarConsulta($sql);
}
