...
switch ($_GET["op"]) {
	...
	case 'borrar':
		$rspta=$departamento->borrar($iddepartamento);
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
            "0"=>'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->iddepartamento.')"><i class="fa fa-pencil"></i></button>'.' '.'<button class="btn btn-danger btn-xs" onclick="borrar('.$reg->iddepartamento.')"><i class="fa fa-minus-circle"></i></button>',
            "1"=>$reg->nombre,
            "2"=>$reg->descripcion,
            "3"=>$reg->fecha_create
              );
